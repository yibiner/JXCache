...
# ===============================================
# Water Service 多节点启动脚本
# 启动 node1(18081), node2(18082), node3(18083)
# ===============================================
...
print_header() {
    echo ""
    echo -e "${BLUE}╔══════════════════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║${NC}${WHITE}          Water Service 多节点启动脚本                ${NC}${BLUE}║${NC}"
    echo -e "${BLUE}║${NC}${DIM}     启动 node1(18081), node2(18082), node3(18083)    ${NC}${BLUE}║${NC}"
    echo -e "${BLUE}╚══════════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
stop_all_nodes() {
    ...
    for i in "${!PROFILES[@]}"; do
        ...
        local pid_file="$PROJECT_DIR/water-$profile.pid"
        
        if [ -f "$pid_file" ]; then
            local pid=$(cat "$pid_file")
            if kill -0 $pid 2>/dev/null; then
                kill $pid 2>/dev/null || true
                echo -e "  ${GREEN}✓${NC} water-$profile (PID: $pid) 已停止"
            fi
            rm -f "$pid_file"
        fi
    done
...
main() {
    ...
    for i in "${!NODES[@]}"; do
        local port=${NODES[$i]}
        echo -e "  curl http://localhost:$port/api/water/$((i+1))"
    done
    echo ""
    echo -e "  ${DIM}# 2. 查看缓存区域${NC}"
    echo -e "  curl http://localhost:18081/api/jxc/observer/areas"
    echo ""
    echo -e "  ${DIM}# 3. 查询缓存数据${NC}"
    echo -e "  curl -X POST 'http://localhost:18081/api/jxc/observer/query' \\"
    echo -e "       -H 'Content-Type: application/json' \\"
    echo -e "       -d '{\"area\":\"default\",\"cacheName\":\"dropletCacheById\"}'"
    echo ""
    ...
    for i in "${!PROFILES[@]}"; do
        local profile=${PROFILES[$i]}
        echo -e "  water-$profile: ${DIM}$PROJECT_DIR/water-$profile.log${NC}"
    done
    echo ""
